...
WHERE   (ContractAddress = @ContractAddress) AND (EventName = @EventName) and   (ChainId = @ChainId)
";
        ...
        public static void UpdateEventLastSysBlock(string contractAddress, string eventName, long lastSynBlockNum, ulong ThisNowBlockNum)
        {
            ...
            if (!DAL.ContEventBlockNum.Exist( ShareParam.DbConStr, chainid, contractAddress, eventName))
            {
