...
namespace BlockChain.Share.BLL
{
    public class Token
    {
        ...
        public static async Task<bool> SaveTokenData(int ChainId, string TokenAddress, string ImagePath, bool FromUser = true, bool isFixedPrice = false, string priceToken = "", decimal price = 0)
        {
            ...
            else
            {
                try
                {
                    ...
                    token.Decimals = service.DecimalsQueryAsync().Result;       //没有这个 decimals function，也不一定抛出异常的！！！  也就是说这里没办法判别这个地址是否是 ERC20Token ！！！
            ...
            else
            {
                ...
                //2022-12-05 太消耗时间了，这里不处理
                token.PricingTokenPrice = 0;                //处理稳定币价格
        ...
        public static System.Data.DataTable GetAllSelectedToken()
        {
            ...
            string sql = @"
SELECT   '" + BasePath + @"' + ImagePath AS ImagePath, 
                (CASE Address WHEN '0x0000000000000000000000000000000000000000' THEN 1 ELSE 0 END) AS IsEth, ChainId, 
                Name, Address, Decimals, Symbol, IsPricingToken, PricingTokenAddress, PricingTokenPrice, 
                PricingTokenPriceUpdateTime, PricingIsFixed
FROM      Token
...
";
        ...
        public static bool SetIsPricingToken(string selectedToken)
        {
            ...
            string sql = @"
update Token set IsPricingToken = 0 where   (ChainId = @ChainId);
update Token set IsPricingToken = 1 where   (Address = @Address)  and   (ChainId = @ChainId);
";
            SqlConnection cn = new SqlConnection(Share.ShareParam.DbConStr);
            SqlCommand cm = new SqlCommand();
            cm.Connection = cn;
