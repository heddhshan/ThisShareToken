...
namespace BlockChain.Share
{
    ...
    public static class ShareParam
    {
        ...
        /// solidity 数据和 dotnet 之间的数据映射，需要单独有一整套！ 那个dotnet版本的geth（Nethermind）是有的，nethereum 也有 （ Nethereum.Unitil.BigDecimal 和 Nethereum.Unitil.BigInteger）！
        /// 同时，微软应该提供 BigDecimal 这样的数据类型
        /// 在使用float（double）这个不准确数据后，会有很多后遗症，例如转账4eth，实际上可能数据变成的4.000000123eth。目前整个系统都存在此类问题，暂时不处理(20220628)。 最麻烦的是 erc20.approve 这个值 少 1 都会导致 transferfrom 失败！
        /// 使用 Nethereum.Unitil.BigDecimal 和 Nethereum.Unitil.BigInteger 可以解决 solidity 精度的问题，但是这两个数据和 dotnet 和 SqlServer 都不完全兼容，也就是说某些运算不能使用这两个数据类型，或者SQL Server需要自定义数据类型才能保存这两种类型。
        ///     所以整个系统还是存在 double ， december 等数据类型 
        /// </summary>
        ...
        {
            return Common.SolidityHelper.getPowerValue(Decimals);
            //switch (Decimals)
            //{
            //    case 0:
            //        return 1;
        ...
        public static int FromBlock
        {
            get
            {
                //return BLL.AppInfo.getFromBlock();                //appinfo 要第一个部署，否则这里会出错！
                return BLL.AppInfo.getFromBlock() - 1_000_000;      // todo: test
            }
        }

        ////币安最多只能查询五千个区块！！！        Nethereum.JsonRpc.Client.RpcResponseException: exceed maximum block range: 5000
        public const int MaxBlock = 5_000_000;         //币安，5000; 以太坊，可以不要; 采用比最大值小点的数据 

        ////public const int NowBlockNum = 16;      //币安，距离现在的区块间隔，币安至少16个区块；以太坊是6个区块
        public const int NowBlockNum = 0;           //todo: 以太坊，距离现在的区块间隔，币安至少16个区块；以太坊是6个区块 测试时候以太坊改为0，否则事件数据很慢才能获取        //以太坊以后采用新的多签算法后，这个值就可以取0！！！
        ...
        public static string AddressAppInfo
        {
            get
            {
                ...
                {
                    return "0x5d6D8E01b259841b878Fa1412D13534e0F2D3eC1";        //https://sepolia.etherscan.io/tx/0x0efcb02c5b76aa4a853e72627b29177732d8df044c68a78d583f56e7693ccca0
                }
                else
                {
        ...
        public static string UserDataDir
        {
            get
            {
                var result = GetUserDataDir();                          //默认是windows账号下面的目录，例如  C:\Users\MyName\Documents\BlockChain\
                if (string.IsNullOrEmpty(result))
                {
                    string BasePath = System.AppDomain.CurrentDomain.BaseDirectory;
                    result = Path.Combine(BasePath, "UserData");        //如果 indows账号下面的目录 创建失败，采用 程序下面 的 UserData 子目录
                }
                return result;
            }
        }
        ...
        /// <summary>
        ///  一次性授权的最大值, 就是 TotalSupply 值！ 不要取一个固定值，例如 10**70，某些Token有限制，会失败！
        /// </summary>
        public static System.Numerics.BigInteger Erc20ApproveMaxValue(string token)
        {
            ...
            return service.TotalSupplyQueryAsync().Result;
        }
        ...
        public static async Task<System.Numerics.BigInteger> GetTokenTotalSupply(string token)
        {
            ...
            catch (Exception ex)
            {
                log.Error("", ex);
                throw ex;
                //return System.Numerics.BigInteger.Pow(10, 50);      //返回这个值肯定是错的
            }
        }
        ...
        /// <returns></returns>
        public static Nethereum.Util.BigDecimal GetRealBalance(string address, string token, bool isCache = false)
        {
            if (IsOfflineWallet == true) { throw new Exception("Require Online Wallet"); }
            try
            {
                ...
                if (isCache)
                {
                    ...
                    if (result != null)
                    {
                        return (Nethereum.Util.BigDecimal)result;
                    }
                }

                Nethereum.Util.BigDecimal amount = 0;
                Nethereum.Web3.Web3 web3 = Share.ShareParam.GetWeb3();

                if (token == "0x0" || Share.ShareParam.IsAnEmptyAddress(token))
                {
                    var big_amount = (web3.Eth.GetBalance.SendRequestAsync(address).Result).Value;
                    amount = Nethereum.Web3.Web3.Convert.FromWeiToBigDecimal(big_amount, 18);               // (double)ShareParam.getPowerValue(18);
                }
                else
                {
                    Nethereum.StandardTokenEIP20.StandardTokenService service = new Nethereum.StandardTokenEIP20.StandardTokenService(web3, token);
                    var big_amount = service.BalanceOfQueryAsync(address).Result;
                    var Decimals = service.DecimalsQueryAsync().Result;

                    amount = Nethereum.Web3.Web3.Convert.FromWeiToBigDecimal(big_amount, Decimals);         //amount / (double)ShareParam.getPowerValue(Decimals);  //Math.Pow(10, Decimals);
                }
        ...
        public static async Task<string> Transfer_Eth(Nethereum.Web3.Accounts.Account from, string to, decimal amount)
        {

            var web3 = Share.ShareParam.GetWeb3(from);
            //BigInteger value = (BigInteger)(amount * (decimal)ShareParam.getPowerValue(18));
            var tx = await web3.Eth.GetEtherTransferService().TransferEtherAsync(to, amount);
            Share.BLL.TransactionReceipt.LogTx(from.Address, tx, "Eth_Transfer", "Eth Transfer, Amount:" + amount.ToString());
            return tx;
        }


        public static async Task<string> Transfer_Erc20Token(Nethereum.Web3.Accounts.Account from, string to, string token, decimal amount)
        {
            if (ShareParam.IsAnEmptyAddress(token))
            {
                return await Transfer_Eth(from, to, amount);
            }

            var web3 = Share.ShareParam.GetWeb3(from);
            Nethereum.StandardTokenEIP20.StandardTokenService service = new Nethereum.StandardTokenEIP20.StandardTokenService(web3, token);
            BigInteger value = (BigInteger)(amount * (decimal)ShareParam.getPowerValue(Share.BLL.Token.GetTokenDecimals(token)));
            ...
            var tx = await service.TransferRequestAsync(param);
            Share.BLL.TransactionReceipt.LogTx(from.Address, tx, "Token_Transfer", "Token Transfer, Amount:" + amount.ToString());
            return tx;
        }


        public static async Task<string> Transfer_Nft(Nethereum.Web3.Accounts.Account from, string to, string nft, BigInteger id)
        {
            var web3 = Share.ShareParam.GetWeb3(from);
            ...
            Share.BLL.TransactionReceipt.LogTx(from.Address, tx, "NFT_Transfer", "NFT Transfer, ID:" + id.ToString());
            return tx;
        }
...
1，开发团队尽力保证整个技术体系的安全，但也可能存在疏漏错误之处。
2，请合法使用本程序，请遵守本国或本地区的相关法律。
3，请勿超过自己的经济和心里承受能力参与各种交易。
无论以上的哪种情况，以及使用本程序造成的各种已知的或未知的风险，团队不承担责任。
";
