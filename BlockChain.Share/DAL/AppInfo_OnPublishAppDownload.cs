...
{
    string sql = @"insert into AppInfo_OnPublishAppDownload (ChainId,ContractAddress,BlockNumber,TransactionHash,_eventId,_AppId,_PlatformId,_Version,_HttpLink,_BTLink,_eMuleLink,_IpfsLink,_OtherLink) values (@ChainId,@ContractAddress,@BlockNumber,@TransactionHash,@_eventId,@_AppId,@_PlatformId,@_Version,@_HttpLink,@_BTLink,@_eMuleLink,@_IpfsLink,@_OtherLink)";
...
{
    string sql = @"Delete From AppInfo_OnPublishAppDownload Where ChainId = @ChainId and ContractAddress = @ContractAddress and _eventId = @_eventId";
    SqlConnection cn = new SqlConnection(conStr);
    SqlCommand cm = new SqlCommand();
    cm.Connection = cn;
    cm.CommandType = System.Data.CommandType.Text;
...
public static Model.AppInfo_OnPublishAppDownload GetModel(string conStr, System.Int32 ChainId, System.String ContractAddress, System.Int64 _eventId)
{
    string sql = @"select * from AppInfo_OnPublishAppDownload Where ChainId = @ChainId and ContractAddress = @ContractAddress and _eventId = @_eventId ";
...
{
    string sql = @"Select Count(*) From AppInfo_OnPublishAppDownload Where ChainId = @ChainId and ContractAddress = @ContractAddress and _eventId = @_eventId ";
    SqlConnection cn = new SqlConnection(conStr);
    SqlCommand cm = new SqlCommand();
    cm.Connection = cn;
    cm.CommandType = System.Data.CommandType.Text;
