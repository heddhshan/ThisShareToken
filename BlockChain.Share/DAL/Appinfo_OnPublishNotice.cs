...
{
    string sql = @"insert into Appinfo_OnPublishNotice (ChainId,ContractAddress,BlockNumber,TransactionHash,_publisher,_appId,_subject,_body,BlockTime) values (@ChainId,@ContractAddress,@BlockNumber,@TransactionHash,@_publisher,@_appId,@_subject,@_body,@BlockTime)";
...
public static int Delete(string conStr, System.Int32 ChainId,System.String TransactionHash)
{
    string sql = @"Delete From Appinfo_OnPublishNotice Where ChainId = @ChainId and TransactionHash = @TransactionHash";
    SqlConnection cn = new SqlConnection(conStr);
    SqlCommand cm = new SqlCommand();
    cm.Connection = cn;
    cm.CommandType = System.Data.CommandType.Text;
...
{
    string sql = @"Update Appinfo_OnPublishNotice Set ContractAddress = @ContractAddress, BlockNumber = @BlockNumber, _publisher = @_publisher, _appId = @_appId, _subject = @_subject, _body = @_body, BlockTime = @BlockTime Where ChainId = @ChainId And TransactionHash = @TransactionHash ";
    SqlConnection cn = new SqlConnection(conStr);
    SqlCommand cm = new SqlCommand();
    cm.Connection = cn;
    cm.CommandType = System.Data.CommandType.Text;
...
public static Model.Appinfo_OnPublishNotice GetModel(string conStr, System.Int32 ChainId, System.String TransactionHash)
{
    string sql = @"select * from Appinfo_OnPublishNotice Where ChainId = @ChainId and TransactionHash = @TransactionHash ";
...
public static bool Exist(string conStr, System.Int32 ChainId, System.String TransactionHash)
{
    string sql = @"Select Count(*) From Appinfo_OnPublishNotice Where ChainId = @ChainId and TransactionHash = @TransactionHash ";
    SqlConnection cn = new SqlConnection(conStr);
    SqlCommand cm = new SqlCommand();
    cm.Connection = cn;
    cm.CommandType = System.Data.CommandType.Text;
